...
export default {
    ...
    created: async function () {
        const response1 = await axios.get('https://cms.ainflow.co.in/api/glossary-categories');
        const sortCat = response1?.data?.data.sort((a, b) => a.id - b.id);
        this.categories = sortCat;
        const response = await axios.get('https://cms.ainflow.co.in/api/glossaries?populate=*')
